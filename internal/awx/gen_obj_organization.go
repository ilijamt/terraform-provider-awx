...
func (o *organizationTerraformModel) setMaxHosts(data any) (d diag.Diagnostics, err error) {
	...
	if val, ok := data.(json.Number); ok {
		...
		o.MaxHosts = types.Int64Value(v)
	} else if val, ok := data.(int64); ok {
		o.MaxHosts = types.Int64Value(val)
	} else {
		o.MaxHosts = types.Int64Null()
	}
	return d, nil
}
...
var (
	_ resource.Resource                = &organizationAssociateDisassociateExecutionEnvironment{}
	_ resource.ResourceWithConfigure   = &organizationAssociateDisassociateExecutionEnvironment{}
	_ resource.ResourceWithImportState = &organizationAssociateDisassociateExecutionEnvironment{}
)

type organizationAssociateDisassociateExecutionEnvironmentTerraformModel struct {
	OrganizationID         types.Int64 `tfsdk:"organization_id"`
	ExecutionEnvironmentID types.Int64 `tfsdk:"execution_environment_id"`
}

// NewOrganizationAssociateDisassociateExecutionEnvironmentResource is a helper function to simplify the provider implementation.
func NewOrganizationAssociateDisassociateExecutionEnvironmentResource() resource.Resource {
	return &organizationAssociateDisassociateExecutionEnvironment{}
}

type organizationAssociateDisassociateExecutionEnvironment struct {
	client   c.Client
	endpoint string
}

func (o *organizationAssociateDisassociateExecutionEnvironment) Configure(ctx context.Context, request resource.ConfigureRequest, response *resource.ConfigureResponse) {
	if request.ProviderData == nil {
		return
	}

	o.client = request.ProviderData.(c.Client)
	o.endpoint = "/api/v2/organizations/%d/execution_environments/"
}

func (o organizationAssociateDisassociateExecutionEnvironment) Metadata(ctx context.Context, request resource.MetadataRequest, response *resource.MetadataResponse) {
	response.TypeName = request.ProviderTypeName + "_organization_associate_execution_environment"
}

func (o organizationAssociateDisassociateExecutionEnvironment) GetSchema(ctx context.Context) (tfsdk.Schema, diag.Diagnostics) {
	return processSchema(
		SourceResource,
		"Organization/Associate",
		tfsdk.Schema{
			Attributes: map[string]tfsdk.Attribute{
				"organization_id": {
					...
					Validators: []tfsdk.AttributeValidator{
						schemavalidator.AlsoRequires(
							path.MatchRoot("execution_environment_id"),
						),
					},
				},
				"execution_environment_id": {
					Description: "Database ID of the executionenvironment to assign.",
					Required:    true,
					Type:        types.Int64Type,
					PlanModifiers: []tfsdk.AttributePlanModifier{
						resource.RequiresReplace(),
...
func (o *organizationAssociateDisassociateExecutionEnvironment) ImportState(ctx context.Context, request resource.ImportStateRequest, response *resource.ImportStateResponse) {
	var state organizationAssociateDisassociateExecutionEnvironmentTerraformModel
	var parts = strings.Split(request.ID, "/")
	var err error
	if len(parts) != 2 {
	...
	var organizationId, executionEnvironmentId int64
	...
	executionEnvironmentId, err = strconv.ParseInt(parts[1], 10, 64)
	if err != nil {
		response.Diagnostics.AddError(
			fmt.Sprintf("Unable to parse '%v' as an int64 number, please provide the executionEnvironment_id for the Organization association.", request.ID),
			err.Error(),
		)
		return
	}
	state.ExecutionEnvironmentID = types.Int64Value(executionEnvironmentId)

	response.Diagnostics.Append(response.State.Set(ctx, &state)...)
}

func (o *organizationAssociateDisassociateExecutionEnvironment) Create(ctx context.Context, request resource.CreateRequest, response *resource.CreateResponse) {
	var err error

	// Retrieve values from state
	var plan, state organizationAssociateDisassociateExecutionEnvironmentTerraformModel
	response.Diagnostics.Append(request.Plan.Get(ctx, &plan)...)
	if response.Diagnostics.HasError() {
		return
	}
	...
	var endpoint = p.Clean(fmt.Sprintf(o.endpoint, plan.OrganizationID.ValueInt64())) + "/"
	var buf bytes.Buffer
	var bodyRequest = associateDisassociateRequestModel{ID: plan.ExecutionEnvironmentID.ValueInt64(), Disassociate: false}
	_ = json.NewEncoder(&buf).Encode(bodyRequest)
	if r, err = o.client.NewRequest(ctx, http.MethodPost, endpoint, &buf); err != nil {
		response.Diagnostics.AddError(
			fmt.Sprintf("Unable to create a new request for Organization on %s for create of type ", o.endpoint),
	...
	state.OrganizationID = plan.OrganizationID
	state.ExecutionEnvironmentID = plan.ExecutionEnvironmentID
...
func (o *organizationAssociateDisassociateExecutionEnvironment) Delete(ctx context.Context, request resource.DeleteRequest, response *resource.DeleteResponse) {
	var err error
	...
	var endpoint = p.Clean(fmt.Sprintf(o.endpoint, state.OrganizationID.ValueInt64())) + "/"
	var buf bytes.Buffer
	var bodyRequest = associateDisassociateRequestModel{ID: state.ExecutionEnvironmentID.ValueInt64(), Disassociate: true}
	_ = json.NewEncoder(&buf).Encode(bodyRequest)
	if r, err = o.client.NewRequest(ctx, http.MethodPost, endpoint, &buf); err != nil {
		response.Diagnostics.AddError(
			fmt.Sprintf("Unable to create a new request for Organization on %s for delete of type ", o.endpoint),
...
func (o *organizationAssociateDisassociateExecutionEnvironment) Read(ctx context.Context, request resource.ReadRequest, response *resource.ReadResponse) {
}

func (o *organizationAssociateDisassociateExecutionEnvironment) Update(ctx context.Context, request resource.UpdateRequest, response *resource.UpdateResponse) {
}
