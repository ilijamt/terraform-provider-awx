...
func (o *notificationTemplateTerraformModel) setMessages(data any) (d diag.Diagnostics, err error) {
	// Decode "messages"
	if val, ok := data.(string); ok {
		o.Messages = types.StringValue(helpers.TrimString(false, false, val))
		...
		o.Messages = types.StringValue(helpers.TrimString(false, false, string(v)))
	} else {
		o.Messages = types.StringNull()
	}
	return d, nil
}
...
func (o *notificationTemplateTerraformModel) setNotificationConfiguration(data any) (d diag.Diagnostics, err error) {
	// Decode "notification_configuration"
	if val, ok := data.(string); ok {
		o.NotificationConfiguration = types.StringValue(helpers.TrimString(false, false, val))
	} else if val, ok := data.(map[string]any); ok {
		var v []byte
		if v, err = json.Marshal(val); err != nil {
			d.AddError(
		...
		o.NotificationConfiguration = types.StringValue(helpers.TrimString(false, false, string(v)))
	} else {
		o.NotificationConfiguration = types.StringNull()
	}
	return d, nil
}

func (o *notificationTemplateTerraformModel) setNotificationType(data any) (d diag.Diagnostics, err error) {
	// Decode "notification_type"
	if val, ok := data.(string); ok {
		o.NotificationType = types.StringValue(helpers.TrimString(false, false, val))
	} else if val, ok := data.(json.Number); ok {
		o.NotificationType = types.StringValue(val.String())
	} else {
		o.NotificationType = types.StringNull()
	}
	return d, nil
}
