...
func (o *hostTerraformModel) setInstanceId(data any) (d diag.Diagnostics, err error) {
	// Decode "instance_id"
	if val, ok := data.(string); ok {
		o.InstanceId = types.StringValue(helpers.TrimString(false, false, val))
	} else if val, ok := data.(json.Number); ok {
		o.InstanceId = types.StringValue(val.String())
	} else {
		o.InstanceId = types.StringNull()
	}
	return d, nil
}
...
func (o *hostTerraformModel) setLastJob(data any) (d diag.Diagnostics, err error) {
	...
	if val, ok := data.(json.Number); ok {
		...
		o.LastJob = types.Int64Value(v)
	} else if val, ok := data.(int64); ok {
		o.LastJob = types.Int64Value(val)
	} else {
		o.LastJob = types.Int64Null()
	}
	return d, nil
}

func (o *hostTerraformModel) setLastJobHostSummary(data any) (d diag.Diagnostics, err error) {
	// Decode "last_job_host_summary"
	if val, ok := data.(json.Number); ok {
		v, err := val.Int64()
		if err != nil {
			d.AddError(
		...
		o.LastJobHostSummary = types.Int64Value(v)
	} else if val, ok := data.(int64); ok {
		o.LastJobHostSummary = types.Int64Value(val)
	} else {
		o.LastJobHostSummary = types.Int64Null()
	}
	return d, nil
}
...
func (o *hostObjectRolesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if r, err = o.client.NewRequest(ctx, http.MethodGet, fmt.Sprintf(o.endpoint, id.ValueInt64()), nil); err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("Unable to create a new request for host"),
			err.Error(),
		)
		return
	}
	...
	if data, err = o.client.Do(ctx, r); err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("Unable to fetch the request for host object roles "),
			err.Error(),
		)
		return
	}
...
func (o *hostAssociateDisassociateGroup) ImportState(ctx context.Context, request resource.ImportStateRequest, response *resource.ImportStateResponse) {
	...
	if len(parts) != 2 {
		...
		response.Diagnostics.AddError(
			fmt.Sprintf("Unable to import state for Host association, invalid format."),
			err.Error(),
		)
		return
	}
