...
resource "awx_credential_aws_secretsmanager_credential" "this" {
  name           = "aws_secretsmanager_credential (updated)"
  organization   = awx_organization.org.id
  aws_access_key = "AKIAUPDATED"
  aws_secret_key = "secret-key"
}

resource "awx_credential_azure_kv" "this" {
  name         = "azure_kv (updated)"
  organization = awx_organization.org.id
...
resource "awx_credential_galaxy_api_token" "this" {
  name         = "galaxy_api_token (updated)"
  organization = awx_organization.org.id
  url          = "https://galaxy.ansible.com/"
  auth_url     = "https://sso.updated.example.com/"
  token        = "galaxy-token"
}
...
resource "awx_credential_github_token" "this" {
  name         = "github_token (updated)"
  organization = awx_organization.org.id
  token        = "github-token-updated"
}

resource "awx_credential_gitlab_token" "this" {
  name         = "gitlab_token (updated)"
  organization = awx_organization.org.id
  token        = "gitlab-token-updated"
}
...
resource "awx_credential_insights" "this" {
  name         = "insights (updated)"
  organization = awx_organization.org.id
  username     = "insights-updated"
  password     = "insights-secret"
}

resource "awx_credential_kubernetes_bearer_token" "this" {
  name         = "kubernetes_bearer_token (updated)"
  organization = awx_organization.org.id
...
resource "awx_credential_rhv" "this" {
  ...
  username     = "rhv-user"
  password     = "rhv-secret"
  ca_file      = "/etc/pki/updated.crt"
}

resource "awx_credential_satellite6" "this" {
  name         = "satellite6 (updated)"
  organization = awx_organization.org.id
  host         = "https://satellite.example.com"
  username     = "sat-updated"
  password     = "sat-secret"
}
...
resource "awx_credential_thycotic_tss" "this" {
  name         = "thycotic_tss (updated)"
  organization = awx_organization.org.id
  server_url   = "https://tss.example.com"
  username     = "tss-user"
  domain       = "tss-domain-updated"
  password     = "tss-secret"
}
